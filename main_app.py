...
def initialize_session_managers():
    ...
    if 'session_managers_initialized' not in st.session_state:
        ...
        st.session_state.session_managers = {
...
def initialize_session_state(managers, force_reset_override=False):
    ...
    # chat_initialized フラグの管理（ループ防止）
    if 'chat_initialized' not in st.session_state:
        st.session_state.chat_initialized = False
        logger.debug("chat_initialized フラグを初期化")

    if force_reset:
        st.session_state.chat_initialized = False
        logger.info("強制リセット: chat_initialized フラグをリセット")
...
def update_background(scene_manager: SceneManager, theme: str):
    ...
    try:
        ...
        css_dict = {
            "default": f"background-image: url('{os.getenv('BACKGROUND_IMAGE_PATH', 'Assets/ribinngu-hiru.jpg')}');",
            "room_night": f"background-image: url('{os.getenv('BACKGROUND_IMAGE_NIGHT', 'Assets/ribinngu-yoru-on.jpg')}');",
            "beach_sunset": f"background-image: url('{os.getenv('BACKGROUND_IMAGE_BEACH_DAY', 'Assets/sunahama-hiru.jpg')}');",
            "festival_night": f"background-image: url('{os.getenv('BACKGROUND_IMAGE_FESTIVAL', 'Assets/maturi-yoru.jpg')}');",
            "shrine_day": f"background-image: url('{os.getenv('BACKGROUND_IMAGE_SHRINE', 'Assets/jinnjya-hiru.jpg')}');",
            "cafe_afternoon": f"background-image: url('{os.getenv('BACKGROUND_IMAGE_CAFE', 'Assets/kissa-hiru.jpg')}');",
            "art_museum_night": f"background-image: url('{os.getenv('BACKGROUND_IMAGE_MUSEUM', 'Assets/bijyutukann-yoru.jpg')}');",
        }
    ...
    if st.session_state.get('letter_generation_completed', False):
        ...
        with col2:
            if st.button("📮 新しい手紙をリクエストする", type="primary", use_container_width=True):
                ...
                # 次回レンダリング時に自動で反映される（st.rerun()削除）
                    ...
                    if form_is_instant and is_database_first_time:
                        ...
                        with st.spinner("初回手紙を生成中..."):
                            try:
                                ...
                                # 画面更新は次回レンダリング時に自動で反映される（st.rerun()削除）
